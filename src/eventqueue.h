...
public:
  ...
  // Public methods
public:
  virtual int AddObserver(const std::string &topic, ObserverCallback callback);
  virtual int AddObserver(const std::string &topic, Observer *instance,
                          ObserverCallbackMethod callback);
  virtual int AddObserver(const std::string &topic,
                          ObserverCallbackFunc callback);
  virtual int AddObserver(const std::string &topic,
                          ObserverDescriptor *observer);

  virtual void RemoveObserver(const std::string &topic,
                              ObserverCallback callback);
  virtual void RemoveObserver(const std::string &topic, Observer *instance,
                              ObserverCallbackMethod callback);
  virtual void RemoveObserver(const std::string &topic,
                              ObserverCallbackFunc callback);
  virtual void RemoveObserver(const std::string &topic,
  ...
  virtual int ResolveTopic(const char *topic);
  virtual int ResolveTopic(const std::string &topic);
  virtual int RegisterTopic(const char *topic);
  virtual int RegisterTopic(const std::string &topic);
  virtual std::string GetTopicByID(int id);
  virtual void ClearTopics();

  virtual void Post(int id, MessagePayload *obj = nullptr,
                    bool autoCleanupPayload = false);
  virtual void Post(std::string topic, MessagePayload *obj = nullptr,
                    bool autoCleanupPayload = false);

protected:
  virtual Message *GetQueueMessage();
  virtual void Dispatch(int id, Message *message);

  virtual ObserverVectorPtr GetObservers(int id);
